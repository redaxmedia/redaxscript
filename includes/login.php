...
function login_post()
{
	...
	if (ATTACK_BLOCKED < 10 && $_SESSION[ROOT . '/login'] == 'visited')
	{
		...
		if ($emailValidator->validate($post_user) == Validator\Validator::FAILED)
		{
	...
	else if ($login_by_email == 0 && $loginValidator->validate($post_user) == Validator\Validator::FAILED)
	{
		$error = l('user_incorrect');
	}
	else if ($login_by_email == 1 && $emailValidator->validate($post_user) == Validator\Validator::FAILED)
	{
		$error = l('email_incorrect');
	}
	else if ($loginValidator->validate($post_password) == Validator\Validator::FAILED
	)
	{
